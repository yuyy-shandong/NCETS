...
ncets_cat <- function(data = data, x1_name = "x1",
    x3_name = "x3", y_name = "y", boots_no = 1000) {

    data <- na.omit(data)
    new_data <- data[, c(y_name, x1_name,
        x3_name)]
    colnames(new_data) <- c("y", "x1",
        "x3")
    ...
    for (i in 1:n_length) {
        ...
        nce_result <- model_nce(new_data,
            cat[i + 1], comdata)

        result_bb <- rep(NA, boots_no)
        for (j in 1:boots_no) {
            chou <- sample(1:dim(new_data)[1],
                dim(new_data)[1], replace = T)
            boost_sample <- new_data[chou,
                ]
            result_bb[j] <- model_nce(boost_sample,
                cat[i + 1], comdata)
        }
        ...
        rownames(result_all) <- paste0(cat[i +
            1], " v.s.", comdata)

        all_result_nce <- rbind(all_result_nce,
            result_all)

    }

    colnames(all_result_nce) <- c("Estimation",
        "2.5%", "97.5%")
...
model_nce <- function(data, big, small) {
    p1 <- data[data$x1 == big & data$x3 ==
        small, ]
    coef1pav <- sum(p1$y) / dim(p1)[1]

    p2 <- data[data$x1 == small & data$x3 ==
        big, ]
